"""
Data ingestion layer. Single entry point so training/serving code never cares
where the data actually comes from (local CSV today, could be a database or
S3 in a real deployment tomorrow).
"""
...
def load_raw_data(path: Path = RAW_DATA_PATH) -> pd.DataFrame:
    """Load raw churn data and do basic schema validation.

    Works with the synthetic dataset generated by generate_synthetic_data.py
    or the real Kaggle Telco Customer Churn CSV (same schema, column names
    normalized to snake_case — see NOTE below if using the raw Kaggle file).
    """
    if not path.exists():
...
def normalize_kaggle_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Rename the raw Kaggle Telco CSV's columns to this project's schema."""
    rename_map = {
